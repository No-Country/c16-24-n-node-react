...
const SidebarItem = ({expanded}) => {
  ...
  let items = [
    ...
    // { id: 4, icon: <LuChefHat size={20} />, text: "Reviews", to: "/reviews"},
      ...
      {items.map((item) => (
        <li key={item.id} className="p-0 md:py-2 md:px-3 md:my-1 font-medium rounded-md cursor-pointer transition-colors group hover:bg-indigo-50 text-gray-600 relative">
          <NavLink to={item.to} className="relative flex items-center text-gray-800 hover:text-blue-400 duration-500" >
            ...
            {!expanded && (
              <div className={`absolute -left-10 -top-10 md:left-full rounded-md px-2 py-1 ml-6 bg-indigo-100 text-indigo-800 text-sm invisible opacity-20 -translate-x-3 transition-all group-hover:visible group-hover:opacity-100 group-hover:translate-x-0`}>
                 {item.text}
              </div>
            )}
          </NavLink>
